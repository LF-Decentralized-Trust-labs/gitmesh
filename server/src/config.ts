...
import { resolveGitmeshAgentsEnvPath } from "./paths.js";
import {
  AUTH_BASE_URL_MODES,
...
if (existsSync(GITMESH_ENV_FILE_PATH)) {
  loadDotenv({ path: GITMESH_ENV_FILE_PATH, override: false, quiet: true });
}

// Development fallbacks: try workspace root, then cwd
...
export function loadConfig(): Config {
  ...
  const strictModeFromEnv = process.env.GITMESH_SECRETS_STRICT_MODE;
  const secretsStrictMode =
    strictModeFromEnv !== undefined
      ? strictModeFromEnv === "true"
      : (fileSecrets?.strictMode ?? false);

  const providerFromEnvRaw = process.env.GITMESH_SECRETS_PROVIDER;
  ...
  const storageProviderFromEnvRaw = process.env.GITMESH_STORAGE_PROVIDER;
  ...
  const storageS3Bucket = process.env.GITMESH_STORAGE_S3_BUCKET ?? fileStorage?.s3?.bucket ?? "gitmesh-agents";
  const storageS3Region = process.env.GITMESH_STORAGE_S3_REGION ?? fileStorage?.s3?.region ?? "us-east-1";
  const storageS3Endpoint = process.env.GITMESH_STORAGE_S3_ENDPOINT ?? fileStorage?.s3?.endpoint ?? undefined;
  const storageS3Prefix = process.env.GITMESH_STORAGE_S3_PREFIX ?? fileStorage?.s3?.prefix ?? "";
  const storageS3ForcePathStyle =
    process.env.GITMESH_STORAGE_S3_FORCE_PATH_STYLE !== undefined
      ? process.env.GITMESH_STORAGE_S3_FORCE_PATH_STYLE === "true"
      : (fileStorage?.s3?.forcePathStyle ?? false);

  const deploymentModeFromEnvRaw = process.env.GITMESH_DEPLOYMENT_MODE;
  const deploymentModeFromEnv =
    deploymentModeFromEnvRaw && DEPLOYMENT_MODES.includes(deploymentModeFromEnvRaw as DeploymentMode)
      ? (deploymentModeFromEnvRaw as DeploymentMode)
      : null;
  const deploymentMode: DeploymentMode = deploymentModeFromEnv ?? fileConfig?.server.deploymentMode ?? "local_trusted";
  const deploymentExposureFromEnvRaw = process.env.GITMESH_DEPLOYMENT_EXPOSURE;
  ...
  const authBaseUrlModeFromEnvRaw = process.env.GITMESH_AUTH_BASE_URL_MODE;
  const authBaseUrlModeFromEnv =
    authBaseUrlModeFromEnvRaw &&
    AUTH_BASE_URL_MODES.includes(authBaseUrlModeFromEnvRaw as AuthBaseUrlMode)
      ? (authBaseUrlModeFromEnvRaw as AuthBaseUrlMode)
      : null;
  const publicUrlFromEnv = process.env.GITMESH_PUBLIC_URL;
  ...
  const disableSignUpFromEnv = process.env.GITMESH_AUTH_DISABLE_SIGN_UP;
  const authDisableSignUp: boolean =
    disableSignUpFromEnv !== undefined
      ? disableSignUpFromEnv === "true"
      : (fileConfig?.auth?.disableSignUp ?? false);
  const allowedHostnamesFromEnvRaw = process.env.GITMESH_ALLOWED_HOSTNAMES;
  ...
  const projectDeletionEnvRaw = process.env.GITMESH_ENABLE_PROJECT_DELETION;
  const projectDeletionEnabled =
    projectDeletionEnvRaw !== undefined
      ? projectDeletionEnvRaw === "true"
      : deploymentMode === "local_trusted";
  const databaseBackupEnabled =
    process.env.GITMESH_DB_BACKUP_ENABLED !== undefined
      ? process.env.GITMESH_DB_BACKUP_ENABLED === "true"
      : (fileDatabaseBackup?.enabled ?? true);
  const databaseBackupIntervalMinutes = Math.max(
    1,
    Number(process.env.GITMESH_DB_BACKUP_INTERVAL_MINUTES) ||
      fileDatabaseBackup?.intervalMinutes ||
      60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    Number(process.env.GITMESH_DB_BACKUP_RETENTION_DAYS) ||
      fileDatabaseBackup?.retentionDays ||
      30,
  );
  const databaseBackupDir = resolveHomeAwarePath(
    process.env.GITMESH_DB_BACKUP_DIR ??
  ...
  return {
    ...
    uiDevMiddleware: process.env.GITMESH_UI_DEV_MIDDLEWARE === "true",
    secretsProvider,
    secretsStrictMode,
